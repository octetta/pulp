...
int skred_command_ctx(char* cmd, struct skode_s *ctx);
...
// dump a wavetable as a chunked base64 stream directly to skred_log/stdout
...
/* Snapshot returns the total number of entries, even when capacity is
 * smaller. Restore clears the current table before installing the copy. */
...
/* Commands are copied as bounded, NUL-terminated templates. */
...
typedef enum {
  ...
  SKRED_CONTROL_EVENT_PATTERN_WAIT = 8,
  SKRED_CONTROL_EVENT_PATTERN_STEP = 9,
  SKRED_CONTROL_EVENT_PATTERN_CHANGE = 10,
  SKRED_CONTROL_EVENT_TEMPO_CHANGE = 11,
  SKRED_CONTROL_EVENT_PATTERN_QUEUE = 12,
  SKRED_CONTROL_EVENT_MUTE_CHANGE = 13,
  SKRED_CONTROL_EVENT_ERROR = 14,
  SKRED_CONTROL_EVENT_PATTERN_DOWNBEAT_SWITCH = 15,
...
/* Direct high-performance C API for live drum pads & performance triggers */
...
} skred_control_response_snapshot_t;
...
/* Snapshot returns the total binding count. Restore keeps dispatch stopped
 * until all bindings have been installed, then applies enabled. */
...
int skred_control_response_restore(
    const skred_control_response_snapshot_t *bindings, int count, int enabled);
