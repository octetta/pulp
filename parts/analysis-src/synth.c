...
int delay_send_set(int voice, int bus, float amount) {
  int index = delay_bus_index(bus);
  if (voice_invalid(voice) || index < 0 || !isfinite(amount))
    return SYNTH_INVALID_VOICE;
  if (amount > 1.0f) amount /= 15.0f;
  sv.delay_bus[voice] = index;
  sv.delay_send[voice] = clampf(amount, 0.0f, 1.0f);
  if (sv.delay_send[voice] <= 0.0f) sv.delay_bus[voice] = -1;
  return 0;
}
...
static void delay_format_bus(char *out, size_t out_size, int index) {
  ...
  snprintf(out, out_size,
    "# DL%d stereo=1 active=%d coarse=%d fine=%d feedback=%d mod_freq=%d mod_depth=%d level=%d time=%.3fms\n",
    index + 1, bus->active, bus->coarse, bus->fine, bus->feedback,
    bus->mod_freq, bus->mod_depth, bus->level, delay_base_ms(bus));
}
...
#define APPEND(...) do { \
    ...
    if (verbose || sv.delay_send[v] > 0.0f) {
        if (sv.delay_bus[v] >= 0)
            APPEND(" ds%d,%g", sv.delay_bus[v] + 1, sv.delay_send[v]);
        else
            APPEND(" ds0,0");
    }
...
int voice_copy(int v, int n) {
  ...
  pan_set(n, sv.pan[v]);
  if (sv.delay_bus[v] >= 0)
    delay_send_set(n, sv.delay_bus[v] + 1, sv.delay_send[v]);
