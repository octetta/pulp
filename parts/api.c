...
#define SKRED_CONTROL_RESPONSE_CAPACITY 256
...
#if defined(__EMSCRIPTEN__)
static int control_dispatch_timer;
#else
...
#if defined(__EMSCRIPTEN__)
  /*
   * Browser workers cannot safely block in select() on the emulated pipe.
   * WASM consumers use polling, and the automatic dispatcher is driven by
   * an Emscripten main-thread timer.
   */
  (void)timeout_ms;
  return 0;
...
#if defined(__EMSCRIPTEN__)
#define SKRED_CONTROL_DISPATCH_INTERVAL_MS 2.0

static void skred_control_dispatch_timer_main(void *user) {
  (void)user;
  if (atomic_load_int(&control_dispatch_stop_requested)) return;
  if (atomic_load_uint64(&control_read_idx) >=
      atomic_load_uint64(&control_write_idx)) return;
...
#if defined(__EMSCRIPTEN__)
  if (control_dispatch_timer) {
    simple_mutex_unlock(&control_dispatch_state_mutex);
    return 0;
  }
  int timer = emscripten_set_interval(skred_control_dispatch_timer_main,
    SKRED_CONTROL_DISPATCH_INTERVAL_MS, NULL);
  if (timer <= 0) {
...
#if defined(__EMSCRIPTEN__)
  int timer = control_dispatch_timer;
  control_dispatch_timer = 0;
  atomic_store_int(&control_dispatch_running, 0);
  simple_mutex_unlock(&control_dispatch_state_mutex);
  if (timer > 0) emscripten_clear_interval(timer);
  return;
#else
  skred_control_event_notify_signal();
  if (control_dispatch_thread_started) {
...
static int skred_scheduled_event_snapshot_cb(int n, uint64_t timestamp,
  ...
  if (snapshot->events && snapshot->count < snapshot->max_events) {
    ...
    out->opcode_stream_mask = event->opcode.stream_mask;
...
static void perf_callback_end(const struct timespec *start,
  ...
  if (elapsed_ns > worst_ns) {
    ...
    uint64_t decayed_worst = worst_ns - (worst_ns >> 10);
    if (decayed_worst < elapsed_ns) decayed_worst = elapsed_ns;
    atomic_store_uint64(&perf_callback_ns_worst, decayed_worst);
  }
...
static ma_engine synth_engine;
static const float* current_capture_input = NULL;

typedef struct {
    ma_data_source_base base;
} skred_ma_data_source;

static ma_result skred_ma_data_source_read(ma_data_source* pDataSource, void* pFramesOut, ma_uint64 frameCount, ma_uint64* pFramesRead)
{
    (void)pDataSource;
    
    synth_capture(&skred_global_engine, (float *)pFramesOut, (float *)current_capture_input, (int)frameCount,
                  AUDIO_CHANNELS, current_capture_input ? AUDIO_CHANNELS : 0, NULL);

    if (pFramesRead) *pFramesRead = frameCount;
    return MA_SUCCESS;
}

static ma_result skred_ma_data_source_get_data_format(ma_data_source* pDataSource, ma_format* pFormat, ma_uint32* pChannels, ma_uint32* pSampleRate, ma_channel* pChannelMap, size_t channelMapCapacity)
{
    (void)pDataSource;
    (void)pChannelMap;
    (void)channelMapCapacity;
    if (pFormat) *pFormat = ma_format_f32;
    if (pChannels) *pChannels = 2;
    if (pSampleRate) *pSampleRate = synth_sample_rate_get();
    return MA_SUCCESS;
}

static ma_data_source_vtable skred_ma_data_source_vtable = {
    skred_ma_data_source_read,
    NULL,
    skred_ma_data_source_get_data_format,
    NULL,
    NULL,
    NULL,
    0
...
static void synth_callback(ma_device* pDevice, void* output, const void* input, ma_uint32 frame_count) {
  ...
  float scratch[8192];
  if (frame_count * pDevice->playback.channels <= 8192) {
      ma_engine_read_pcm_frames(&synth_engine, scratch, frame_count, NULL);
  }
#endif
...
static int audio_init_selected_device(unsigned int sample_rate) {
  ...
  engineConfig.channels = synth_device.playback.channels;
  engineConfig.sampleRate = synth_device.sampleRate;
  if (ma_engine_init(&engineConfig, &synth_engine) != MA_SUCCESS) {
      ma_device_uninit(&synth_device);
      return -1;
  }
  
  ma_data_source_config sourceConfig = ma_data_source_config_init();
  sourceConfig.vtable = &skred_ma_data_source_vtable;
  if (ma_data_source_init(&sourceConfig, &synth_data_source.base) != MA_SUCCESS) {
      ma_engine_uninit(&synth_engine);
      ma_device_uninit(&synth_device);
      return -1;
  }
  // Currently the sound source is just standing by; we will need a ma_sound or node to connect it later
#endif
...
void skred_process_stereo(float *out_left, float *out_right, unsigned int frame_count) {
  skred_process_stereo_with_input(out_left, out_right, NULL, NULL, frame_count);
}
...
void skred_process_stereo_with_input(float *out_left, float *out_right, const float *in_left, const float *in_right, unsigned int frame_count) {
  ...
  float in_buffer[HEADLESS_MAX_CHUNK * 2];
  ...
  while (offset < frame_count) {
      ...
      if (in_left || in_right) {
          ...
          for (unsigned int i = 0; i < chunk; i++) {
              in_buffer[i * 2] = in_left ? in_left[offset + i] : 0.0f;
              in_buffer[i * 2 + 1] = in_right ? in_right[offset + i] : 0.0f;
          }
      }
      
      internal_synth_process(buffer, in_channels ? in_buffer : NULL, (int)chunk, 2, in_channels, NULL);
      
      for (unsigned int i = 0; i < chunk; i++) {
          if (out_left) out_left[offset + i] = buffer[i * 2];
          if (out_right) out_right[offset + i] = buffer[i * 2 + 1];
      }
      offset += chunk;
  }
}
...
int skred_execute_opcode(int voice, int opcode, int argc, const double *args) {
  ...
  return skode_execute_voice_opcode(&event, voice);
}
